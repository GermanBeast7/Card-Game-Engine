...
import engine.card.BasicCard.Suit;
import engine.card.BasicCard;
import engine.hand.*;
...
public class BasicDeckInteractions {
	...
	/**
	 * Method to draw a number of cards into a hand
	 * @param deck Deck to draw cards from
	 * @param hand Hand to give cards from the deck
	 * @param numberToGive the number of cards to draw
	 */
	public static void draw(Deck<BasicCard> deck, Hand<BasicCard> hand, int numberToGive) {
		for(int count = 0; (count < numberToGive) && (!deck.isEmpty());count++) {
			hand.add(deck.draw());
		}
	}
